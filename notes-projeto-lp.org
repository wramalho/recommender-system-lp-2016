...
2. Certifique-se que o /path/ da variável =my-beaches= está de acordo com a localização do arquivo praias.csv em sua máquina
3. Use a função =get-recommendations= no REPL do seguinte modo: =(get-recommendations praias-waldo)=
4. A variável praias-waldo pode ser substituída por qualquer lista associativa com as preferências do usuário,
